...
export default function Problem({
  ...
  if (!testData) return <div className={styles.sectionContent}>Loading problem…</div>;
  if (!questions.length) return <div className={styles.sectionContent}>No questions available.</div>;
  ...
  return (
    <div className={styles.problemWrapper}>
      ...
      <div className={styles.questionNav}>
        {questions.map((_, i) => (
          <button
            key={i}
            onClick={() => setIdx(i)}
            className={`${styles.questionDot} ${i === idx ? styles.activeDot : ""}`}
          >
            {i + 1}
          </button>
        ))}
      ...
      <div className={styles.questionCard}>
        ...
        <div className={styles.questionStatement}>{question.statement}</div>

        <div className={styles.constraintsBox}>
          <h4 className={styles.subTitle}>Constraints:</h4>
          <div className={styles.constraintsText}>{question.constraints}</div>
        </div>
